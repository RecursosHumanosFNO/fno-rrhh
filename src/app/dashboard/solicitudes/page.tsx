...
import { solicitudes as allSolicitudes, empleados } from '@/lib/mockData'
import {
  SOLICITUD_TIPO_LABEL, SOLICITUD_ESTADO_COLOR, SOLICITUD_ESTADO_LABEL, formatFecha,
} from '@/lib/utils'
import type { Solicitud, SolicitudTipo } from '@/types'
import {
  ClipboardList, Plus, Search, X, CheckCircle2, XCircle, Clock,
  MessageSquare, Paperclip, ChevronDown, ChevronUp, Filter,
} from 'lucide-react'
...
export default function SolicitudesPage() {
  ...
  const base = isAdmin
    ? allSolicitudes
    : allSolicitudes.filter(s => s.empleadoId === user?.empleadoId)
    ...
    <div className="page-container">
      ...
      {filtered.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          {!isAdmin && <p className="text-slate-400 text-sm mt-1">Creá tu primera solicitud arriba.</p>}
        </div>
      ) : (
                  ...
                  <div className="border-t border-slate-100 dark:border-slate-800 p-4 space-y-4 bg-slate-50/50 dark:bg-slate-800/30 animate-fade-in">
                    ...
                    {sol.comentarioAdmin && (
                      <div className="bg-brand-50 dark:bg-brand-900/20 rounded-xl p-3 border border-brand-100 dark:border-brand-800">
                        <p className="text-xs font-semibold text-brand-700 dark:text-brand-400 mb-1">Respuesta de RRHH</p>
                    ...
                    {isAdmin && sol.estado === 'pendiente' && (
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <button className="btn-success flex-1 justify-center">
          ...
          <div className="card w-full max-w-lg max-h-[85vh] overflow-y-auto animate-scale-in" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="form-label">Fecha inicio *</label><input className="form-input" type="date" /></div>
                <div><label className="form-label">Fecha fin</label><input className="form-input" type="date" /></div>
              </div>
              <div>
                <label className="form-label">Descripción / Motivo *</label>
                <textarea className="form-input resize-none" rows={3} placeholder="Describí brevemente el motivo..." />
              </div>
              <div>
                <label className="form-label">Adjuntar archivo (opcional)</label>
                <div className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-4 text-center cursor-pointer hover:border-brand-500 transition-colors">
                  <Paperclip className="w-6 h-6 text-slate-400 mx-auto mb-1" />
                  <p className="text-sm text-slate-500">Certificado médico, comprobante, etc.</p>
                  <p className="text-xs text-slate-400 mt-0.5">PDF, JPG o PNG — máx. 5MB</p>
                </div>
              </div>
