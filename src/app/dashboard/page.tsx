  ...
  return (
    <div className="page-container">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Registros de Novedad — widget principal */}
        <div className="lg:col-span-2 card overflow-hidden">
          <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div>
              <p className="section-title">Últimos Registros de Novedad</p>
              <p className="section-subtitle">{registrosNovedad.length} registros en total</p>
            </div>
            <Link href="/dashboard/novedades-internas" className="text-sm text-brand-600 dark:text-brand-400 hover:underline font-medium flex items-center gap-1">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {registrosNovedad.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm">Sin registros de novedad aún</p>
              </div>
            ) : registrosNovedad.slice(0, 5).map(reg => (
              <div key={reg.id} className="p-4 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                <div className="w-9 h-9 rounded-full bg-brand-700 flex items-center justify-center text-white text-xs font-bold shrink-0">
                  {reg.empleadoNombre.split(' ').slice(0, 2).map(n => n[0]).join('')}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 truncate">
                    {reg.sector} · {reg.fecha}
                  </p>
                </div>
                <span className={`badge text-xs shrink-0 hidden sm:inline-flex ${REGISTRO_NOVEDAD_CATEGORIA_COLOR[reg.categoria]}`}>
                  {REGISTRO_NOVEDAD_CATEGORIA_LABEL[reg.categoria]}
                </span>
                <Link
                  href="/dashboard/novedades-internas"
      ...
      {/* Solicitudes pendientes — compacto */}
      {pendientes.length > 0 && (
        <div className="card overflow-hidden">
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <ClipboardList className="w-4 h-4 text-amber-500" />
              <p className="font-semibold text-slate-700 dark:text-slate-200 text-sm">Solicitudes pendientes</p>
              ...
              return (
                <div key={sol.id} className="px-4 py-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 truncate">{SOLICITUD_TIPO_LABEL[sol.tipo]}</p>
                  </div>
                </div>
              )
            })}
